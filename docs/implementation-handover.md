...
- R4 is fully closed out (live load, row-level content, and idempotent
  replay all confirmed). **R5** (`dashboards/snowflake_loader.py`,
  `dashboards/pages/snowflake_history.py`, a new read-only `reader`
  Snowflake role in `workspace/main.tf`) is implemented and unit tested,
  including a headless Streamlit smoke test of the failure/demo-banner
  path, but **not yet run against the live account** - the `reader`
  role/grants haven't been applied yet. That's the next concrete action:
  re-run `Deploy Snowflake Platform` to create the `reader` role, then open
  the dashboard's "Snowflake History" page with real credentials and
  confirm it shows `st.success` with real rows, not the demo banner.
...
| Streamlit integration/live cache | R5 historical view implemented/local | [Loader](../dashboards/snowflake_loader.py), [page](../dashboards/pages/snowflake_history.py), [tests](../tests/unit/test_snowflake_loader.py); new read-only `reader` role in [workspace/main.tf](../snowflake/terraform/workspace/main.tf) not yet applied live; live cache (R7) still planned |
...
| R5 | One historical Streamlit view using Snowflake | ~~Implemented (2026-09-26)~~: `dashboards/snowflake_loader.py` + `dashboards/pages/snowflake_history.py`, a new read-only `reader` Snowflake role (separate from the R4 loader role) added to `workspace/main.tf`, 5-minute cached queries, explicit `LoadStatus` banner (never masks a failed connection as demo success), old dashboard pages/paths untouched. **Not yet run live** - the `reader` role/grants haven't been applied yet, and the dashboard has never queried the real account. Done when the live "Snowflake History" page shows `st.success` with real rows, not the demo banner |
