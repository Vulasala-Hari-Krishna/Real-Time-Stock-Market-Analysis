...
- **Current position: R1, R2, and R3's core objectives are DONE (2026-09-25/26);
  R4's loader had its first live run (2026-09-26), found and fixed one real
  bug, and is not yet re-run.** The Databricks Free Edition platform
  bootstrap, the quote-processing job (Auto Loader -> bronze -> silver ->
  gold, on serverless compute), the gold snapshot exporter
  ...
  tables), and the Snowflake platform bootstrap (storage integration,
  warehouse, database, schema, non-admin loader role granted to a real
  user, `SERVING` schema, external stage over `publish/`) have all run live
  and succeeded, via five fully automated GitHub Actions workflows
  (`deploy-databricks-platform.yaml`, `deploy-databricks-job.yaml`,
  `export-gold-snapshot.yaml`, `deploy-snowflake-platform.yaml`, plus the
  AWS-only ones). A published batch (Parquet + manifest.json) exists in
  `publish/batches/` on S3, and a Snowflake trial account (`ILMRWBU-TX52777`)
  now has the full `workspace` root applied, including the `SERVING` schema
  and the loader role's user grant. **R4** (`src/load/snowflake_snapshot.py`,
  versioned SQL DDL in `snowflake/sql/`, `load-snowflake-snapshot.yaml`)
  reached real Snowflake-adjacent logic on its first live run - it correctly
  found the latest batch, read the real manifest, and then correctly
  rejected it: the manifest's real columns include a `bronze_version`
  lineage column this loader's hand-written expected-schema registry didn't
  know about. Fixed (registry + both DDL files + a regression test); **not
  yet re-run**. Raw landing (local, opt-in) remains implemented but not yet
  exercised against live Kafka. See [snowflake/README.md](../snowflake/README.md).
...
- The next task is **re-running R4's loader live** (see "Remaining Roadmap"
  below) - the schema-drift bug is fixed, but the run has never gotten past
  it, so `COPY INTO`, the atomic publish transaction, and the ledger MERGE
  are all still unexercised against the real account. Do not declare R4
  done until a live `load-snowflake-snapshot.yaml` run has been observed to
  populate `SERVING.DAILY_QUOTE_SUMMARY` and record a `completed`
  `BATCH_LEDGER` row.
...
| R4 | Snowflake snapshot loader and analytical model | ~~Implemented and unit tested (2026-09-26)~~: exact manifest files load to staging; key/schema/count checks; atomic DML publication plus `BATCH_LEDGER` batch ledger; stale/duplicate-version rejection and idempotent replay covered. **Not yet run live** - done when a real `load-snowflake-snapshot.yaml` run populates `SERVING.DAILY_QUOTE_SUMMARY` and a second run of the same batch is confirmed as a no-op |
