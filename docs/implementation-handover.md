...
- **Current position: R1 and R2's core objectives are DONE (2026-09-25).**
  The Databricks Free Edition platform bootstrap, the quote-processing job
  (Auto Loader -> bronze -> silver -> gold, on serverless compute), and the
  gold snapshot exporter (`src/export/gold_snapshot.py`, reading via
  DuckDB's delta extension since `deltalake` cannot read Databricks' default
  deletionVectors-enabled tables) have all run live and succeeded against the
  real project bucket/workspace in `us-east-2`, via three fully automated
  GitHub Actions workflows (`deploy-databricks-platform.yaml`,
  `deploy-databricks-job.yaml`, `export-gold-snapshot.yaml`). A published
  batch (Parquet + manifest.json) exists in `publish/batches/`. Raw landing
  (local, opt-in) remains implemented but not yet exercised against live
  Kafka. Snowflake integration is not implemented — that's next (R3/R4).
...
- The next task is **R3: Snowflake platform IaC and identity bootstrap** (see
  "Remaining Roadmap" below), not silently adding more services or declaring
  the whole pipeline done — R1/R2 are the Databricks-side slices, not the
...
| R3 | Snowflake platform IaC and safe identity bootstrap | **IaC written and offline-validated (cfn-lint, YAML syntax) 2026-09-25**; no real apply yet. `snowflake/terraform/main.tf` + [deploy-snowflake-platform.yaml](../.github/workflows/deploy-snowflake-platform.yaml) automate the full bootstrap (mirrors the Databricks two-phase IAM trust pattern in one root instead of two). Two schema points are genuinely unverified against a live apply (see `snowflake/terraform/README.md`): `describe_output[0]` indexing on the storage integration, and the `on_schema_object` grant block for the stage. Done when: a real apply succeeds (with at most one fix-iteration on those two points), the loader role is confirmed non-admin, cost controls are confirmed live, and a destroy path exists (deliberately deferred until after the first real apply) |
