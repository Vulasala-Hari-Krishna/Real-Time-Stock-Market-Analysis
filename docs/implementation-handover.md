...
- **Current position: R1, R2, and R3's core objectives are DONE (2026-09-25/26);
  R4's loader is implemented but not yet run live (2026-09-26).**
  The Databricks Free Edition platform bootstrap, the quote-processing job
  (Auto Loader -> bronze -> silver -> gold, on serverless compute), the gold
  snapshot exporter (`src/export/gold_snapshot.py`, reading via DuckDB's
  delta extension since `deltalake` cannot read Databricks' default
  deletionVectors-enabled tables), and the Snowflake platform bootstrap
  (storage integration, warehouse, database, schema, non-admin loader role,
  external stage over `publish/`) have all run live and succeeded, via five
  fully automated GitHub Actions workflows (`deploy-databricks-platform.yaml`,
  `deploy-databricks-job.yaml`, `export-gold-snapshot.yaml`,
  `deploy-snowflake-platform.yaml`, plus the AWS-only ones). A published
  batch (Parquet + manifest.json) exists in `publish/batches/` on S3, and a
  Snowflake trial account (`ILMRWBU-TX52777`) now has a working external
  stage pointed at it. Raw landing (local, opt-in) remains implemented but
  not yet exercised against live Kafka. **R4** (the Snowflake snapshot
  loader - `src/load/snowflake_snapshot.py`, versioned SQL DDL in
  `snowflake/sql/`, a new `SERVING` schema added to the `workspace`
  Terraform root, and `load-snowflake-snapshot.yaml`) is written and unit
  tested (402 passed) but has never actually run against the live account -
  that's the next concrete action, starting with a `workspace` re-apply to
  pick up the new schema/grant. See [snowflake/README.md](../snowflake/README.md).
...
- The next task is **running R4's loader live for the first time** (see
  "Remaining Roadmap" below) - it exists and is unit tested, but nothing has
  actually loaded real data into Snowflake yet. Do not declare R4 done
  until a live `load-snowflake-snapshot.yaml` run has been observed to
