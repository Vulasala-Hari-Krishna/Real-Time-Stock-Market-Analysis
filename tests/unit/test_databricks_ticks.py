...
from src.batch import databricks_ticks as job
from src.batch import landed_ticks


@pytest.fixture()
def config() -> job.TickJobConfig:
    return job.TickJobConfig(
        catalog="portfolio", schema_prefix="stocks", bucket="test-bucket"
    )


def test_config_is_explicit_and_paths_are_isolated(config: job.TickJobConfig) -> None:
    assert config.table("raw") == "portfolio.stocks_bronze.ticks_raw"
    assert (
        config.path("raw")
        == "s3://test-bucket/lakehouse/bronze/portfolio/stocks/ticks_raw"
    )
    assert (
        config.checkpoint
        == "s3://test-bucket/checkpoints/hybrid/portfolio/stocks/ticks_raw"
    )
...
def test_invalid_job_parameters(
    config: job.TickJobConfig, field: str, value: object
) -> None:
    with pytest.raises(ValidationError):
        job.TickJobConfig(**{**config.model_dump(), field: value})


@pytest.mark.parametrize(
    "mode", ["missing", "valid", "wrong_format", "wrong_path", "no_detail"]
)
def test_registered_location_guard(config: job.TickJobConfig, mode: str) -> None:
    spark = MagicMock()
...
def test_missing_history_fails() -> None:
    ...
    with pytest.raises(ValueError, match="No Delta history"):
        job.table_version(spark, "portfolio.stocks_bronze.ticks_raw")
    spark.sql.return_value.first.return_value = {"version": 4}
    assert job.table_version(spark, "portfolio.stocks_bronze.ticks_raw") == 4


@pytest.mark.parametrize("finishes", [True, False])
def test_available_now_and_timeout(config: job.TickJobConfig, finishes: bool) -> None:
    spark = MagicMock()
    ...
    reader.load.assert_called_once_with("s3://test-bucket/landing/ticks/")
...
def test_snapshot_write_does_not_evolve_schema(config: job.TickJobConfig) -> None:
    frame = MagicMock()
...
def test_noop_requires_intact_completed_versions(
    config: job.TickJobConfig, mode: str
) -> None:
    spark = MagicMock()
...
@pytest.mark.parametrize(
    "failure", [None, "row_limit", "counts", "duplicate", "gold", "write"]
)
def test_publication_state_is_completed_only_after_validated_outputs(
    config: job.TickJobConfig, failure: str | None
) -> None:
    spark = MagicMock()
    ...
    silver = classified.filter.return_value.drop.return_value
    silver.groupBy.return_value.count.return_value.filter.return_value.limit.return_value.count.return_value = (
        1 if failure == "duplicate" else 0
    )
    gold = MagicMock()
    gold.agg.return_value.first.return_value = {"total": 0 if failure == "gold" else 1}
    with patch.object(job, "classify_ticks", return_value=classified), patch.object(
        job, "summarize_quotes", return_value=gold
    ), patch.object(job, "write_snapshot") as write, patch.object(
...
def test_retry_after_ingestion_recovers_unfinished_outputs(
    config: job.TickJobConfig, current: bool
) -> None:
    spark = MagicMock()
...
def test_spark_expression_builders_require_no_platform_clients() -> None:
    frame = MagicMock()
    with patch.object(landed_ticks, "F") as functions, patch.object(
        landed_ticks, "Window"
    ):
        functions.col.return_value.__gt__.return_value = MagicMock()
        landed_ticks.classify_ticks(frame)
        functions.udf.assert_called_once_with(
            landed_ticks.normalize_envelope, landed_ticks.NORMALIZED_SCHEMA
        )
        landed_ticks.summarize_quotes(frame)
        functions.max_by.assert_any_call(
            "reported_volume", functions.struct.return_value
        )
...
def test_bundle_is_manual_bounded_and_uses_explicit_platform_inputs() -> None:
    ...
    deployed = bundle["resources"]["jobs"]["landed_ticks"]
    ...
    assert (
        ...
        == "src.batch.databricks_ticks:main"
    )
    assert not any(
        "spark" in dependency or "delta" in dependency
